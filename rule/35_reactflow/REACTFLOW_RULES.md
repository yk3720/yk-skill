...
**企画・ADR:** [`specs/03_技術仕様/意思決定記録(ADR).md`](c:/yk-application/flowchart-studio/specs/03_技術仕様/意思決定記録(ADR).md)（ADR-006 · ADR-010）
...
**最終更新:** 2026-06-24（L0 glob · 02_spec 廃止 · マルチルート）  
...
| 段階 | 読むもの | タイミング |
|------|----------|------------|
| L0 | `reactflow-dev-entry.mdc` | `flowchart-studio` · `flowchart-web-mermaid` の `lib/flowchart` · `components/flowchart` 編集時 |
| L1 | **本ファイル** | 毎回・最初 |
...
**対象:** `flowchart-studio` のエディタ UI · 3 ペイン · 表→再生成の日常操作。企画: [相談_2026-05-30_Web版ブラッシュアップ方針.md](c:/yk-application/flowchart-studio/specs/01_要求定義/相談_2026-05-30_Web版ブラッシュアップ方針.md)。
...
| ボタン・ダイアログ・ナビ・表 chrome class | `components/flowchart/flowchartUiClasses.ts`（`fcBtn*` · `fcTable*` · `fcColorLegend*` · バナー · `fcFitViewOptions`） |
| 開発用見本ページ | `app/dev/style/` — `npm run dev` 時 `http://localhost:3000/dev/style`（本番 **404**） |
| 表ペイン : プレビュー比率 | `FC_WORKSPACE_MAIN_GRID` = **`2fr:3fr`**（`lg:grid-cols-[2fr_3fr]`） |
| 左ナビ幅 | `min(20%, 240px)` 程度（`ModuleNavPane`） |
| 枠線太さ | [`VISUAL_DESIGN_RULES.md`](../10_meta/VISUAL_DESIGN_RULES.md) §2 と整合 |

**禁止:** フロー編集 chrome に `blue-600` 等の Tailwind 色を直書き（`flowchartUiClasses.ts` 経由）。キャンバス色は `flowColors.ts`（§5.6-3）— chrome と混在させない。
...
| **正規化 SSOT** | **Python** `flowchart-studio/tools/excel_normalize/` → `import.json`（Power Query 任意 · VBA 禁止） |
...
| **Git（作者 xlsx）** | **`*.xlsx` / `*.xls` はコミットしない** — ローカル編集 · 共有は **`import.json`**（`tools/excel_normalize/.gitignore`）· CI は `excel:template` / `excel:fixture` で生成 |
| **設計 SSOT** | [Excel取込_正規化パイプライン.md](c:/yk-application/flowchart-studio/specs/03_技術仕様/Excel取込_正規化パイプライン.md) |
...
| 項目 | SSOT |
|------|------|
| **作者向け記述（図形 · 色 · ループ）** | [`flowchart-studio/specs/03_技術仕様/フローチャート記述ルール.md`](c:/yk-application/flowchart-studio/specs/03_技術仕様/フローチャート記述ルール.md)（§4 ループ · 図形5種 · 色4種） |
...
| **表の書き方（作者）** | [`flowchart-studio/specs/03_技術仕様/フローチャート記述ルール.md`](c:/yk-application/flowchart-studio/specs/03_技術仕様/フローチャート記述ルール.md) **§4**（No → 右列1行 → 接続先(下) で戻す） |
