...
| 表ペイン **Excelから取込…**（`parseExcelBuffer`） | **v1 行のまま表に入る**可能性 — UI は v2 ヘッダー |

| 症状 | 対処 |
|------|------|
| 取込後 **接続先エラー** · Text/MR が列ずれ | **repair スクリプトは作らない** — 取込直後に `migrateTable10ColV1ToV2` / `normalizeFlowchartDocument` を通す **アプリ修正**（未実装時は `_scratch` xlsx 再生成 + E2E skip で明示） |
