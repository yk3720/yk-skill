...
**最終更新:** 2026-09-15（5.6-7c 訂正: ルーラーの区切りは座標ではなく level/tier の論理キーでグルーピングする — 座標グルーピングは〇の中央寄せ・判断の拡大で列がズレて重なるバグを実機で確認）。旧: 2026-09-15 初版（グリッドトラック共有パターン）· 2026-07-25（ハンドル制約 · centerY 幹線スタイル統一 · MIN_TIER_GAP_V 追記）
...
### 5.6-7c パン・ズームに追従するルーラー（`flowchart-excel` 2026-09-15）

**やりたいこと:** Excel の行/列見出しのような、段（tier）・列（level）の番号を表示する固定バーをキャンバスの上端・左端に置き、パン・ズームでも正しい位置に追従させる。

**パターン:** `useViewport()`（`@xyflow/react`）が返す `{x, y, zoom}` を、ルーラー用セルの**そのままのローカル座標**として使える設計にする。CSS Grid で角セル＋列ルーラー（上段）・行ルーラー＋本体（下段）の 2×2 を組み、**列ルーラーと本体キャンバスは同じ列トラック**、**行ルーラーと本体は同じ行トラック**を共有させる（`gridTemplateColumns`/`gridTemplateRows` は `[ルーラー幅]px 1fr` の2トラックのみ）。トラックを共有すれば、ルーラーセルと ReactFlow 本体の原点が一致するため、`left: viewportX + node.x * zoom` のような座標変換なしでそのまま位置が合う。トラックがズレていると原点オフセット分の補正計算が必要になり複雑化する。

**ノードの段・列区切りの求め方 — 見た目の座標(x/y)ではなく論理キー(level/tier)でグルーピングする（落とし穴・2026-09-15 訂正）:** 当初「同じ開始座標(x/y)を持つノードを1区画にまとめる」実装にしたところ、実機で**列2以降のルーラー番号が重なって読めなくなる**バグが出た。原因は `layoutGrid` の配置ロジックが図形種別ごとに座標をずらすこと — **〇（省略記号）は列内で中央寄せ・縮小**されて `x` が列の左端よりずれ、**判断（菱形）は行より1.3倍に拡大**されて `y` が行の上端よりマイナス方向にずれる。そのため座標だけを見ると、同じ論理的な列/段のノードでも別区画に分かれてしまう。

**正しいパターン:** グルーピングは座標ではなく、ノードが本来持つ**論理キー（`level`/`tier` の値そのもの）**で行う。区画の `start` はグループ内の**最小値**、`size` は**最大値**を採用すれば、中央寄せ・拡大された図形が単独でも境界がズレない（`lib/flowchart/graph/rulerSegments.ts` `computeRulerSegments` はキー抽出・start抽出・size抽出の3関数を受け取る設計）。`level` は `FlowNode` に必須で既にあるが、`tier` は8列表では未設定（`undefined`）のことがあるため、`nodeTier(node)`（`graph/buildEdges.ts` の `tier ?? rowIndex` フォールバックを export）で解決してから `FlowNodeData` に持たせる（`toReactFlow.ts`）。区切りの順序は `start` の昇順ソートで求め、**1始まりの連番**を振る（tier/level の生の値ではなく「見た目の並び順」を表示する）。

**教訓:** 図形が「表示上は同じセルに属するが、個別に座標変換（中央寄せ・拡大・縮小）されている」場合、その座標を直接キーにしたグルーピングは信頼できない。グルーピングには必ず変換前の論理値を使う。

**エクスポート除外:** ルーラーは `data-flowchart-export-root` の**外**（兄弟）に置く。PNG/SVG エクスポートは `.react-flow__viewport` を `html-to-image` で捕捉するため、ルーラーを export-root の中に入れると毎回パン位置に依存した中途半端な数字が画像に写り込む（ズーム UI が export-root の外に置かれているのと同じ理由、§4 参照）。
