...
**引き継ぎ終了 Phase C:** **Bash ツール** + **`&&` 連結**（[git-save.md](../../.claude/skills/handoff-session-work/references/git-save.md)）。PowerShell HEREDOC commit **禁止**。
...
- **引き継ぎ Phase C:** リポごと **Bash 1 本**（add+commit+push）。2 リポ = **2 Run**（並列可）· Post-C 専用 commit **禁止**。
...
```text
引き継ぎして。commit+push まで。Run はリポごとに1回。Post-C の追加 commit は不要。
```

```text
yk-memo だけコミット＆プッシュ。git は Bash 1 本（add+commit+push）· 初回からフル権限で。
```
