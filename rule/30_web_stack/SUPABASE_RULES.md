...
**ステータス:** `active`（[`RULE_INDEX.md`](../RULE_INDEX.md) No 37 · PROGRESSIVE L0/L1/L2 整備済）— **RLS · Auth · env の MUST は本ファイルと L3 references に従う**。

**実行手順・Ref Plan:** `.claude/skills/creating-supabase-yk/SKILL.md`  
**ロード規則 SSOT:** `.../creating-supabase-yk/references/ROUTER.md`  
**L0 入口:** `yk-skill/.cursor/rules/supabase-dev-entry.mdc`

**実装スタック:** `flowchart-studio`（ADR-013 DB-1）· [`NEXTJS_RULES.md`](NEXTJS_RULES.md) · [`REACTFLOW_RULES.md`](../35_reactflow/REACTFLOW_RULES.md)

**最終更新:** 2026-06-27（P15 PROGRESSIVE 完遂 · L0/L2/ROUTER）
...
| 段階 | 読むもの | タイミング |
|------|----------|------------|
| L0 | `supabase-dev-entry.mdc` | `lib/supabase/**` · `app/auth/**` · migrations 編集時 |
| L1 | **本ファイル** | Supabase / Auth / RLS を触るたび・最初 |
| L2 | `creating-supabase-yk/SKILL.md` | 手順 · Shell 停止条件 |
| L2.5 | **Ref Plan** | **Write/StrReplace 前**（ROUTER §7） |
| L3 | ROUTER で列挙した `references/` のみ | Ref Plan 後 |

**tier / tag:** → `creating-supabase-yk/references/ROUTER.md` §2〜§4（本ファイルに表を置かない）
