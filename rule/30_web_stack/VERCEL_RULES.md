...
- **`--scope` に Personal Account 名を指定すると `You cannot set your Personal Account as the scope` エラー**になる。Personal Account（`ykoba56-1226` 等）は scope に指定できない。**`--scope` なしで実行**するとチームを自動選択する。
