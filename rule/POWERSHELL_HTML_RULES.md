...
### なぜ重要か
`[System.IO.File]::WriteAllText(path, content)` の **2引数版**は、**Windows PowerShell 5.1（.NET Framework）** では **UTF-8（BOM 付き）で書き込むことが多い**。**PowerShell 7（.NET 5+）** では **BOM なし UTF-8** になるなど、**環境で既定が異なる**。  
HTML を配布・ブラウザ表示する用途では、**先頭バイトが `<`（例: `60 33 68` = `<!D`）から始まる BOM なし UTF-8**にそろえると、**BOM 起因の表示ずれ**を避けやすい（ブラウザや `<meta charset>` との組み合わせは環境依存のため、**再現したらバイト列で確認**する）。
