...
**flowchart-web-* の `app/` のみ:** L1 §6 + 本スキル。`components/flowchart` · `lib/flowchart` は **`creating-reactflow-yk`**（ROUTER tag `flowchart-web` は RF 作業時のみ）。
...
| パス | 優先 entry |
|------|------------|
| `flowchart-web-*/app/**` | **`reactflow-dev-entry.mdc`**（Next シェルは本スキル） |
