...
Phase A を飛ばして Phase B だけ行った状態は **引き継ぎ終了済ではない**（要整理）。Phase B だけで Phase C を省略した状態も **引き継ぎ終了済ではない**（Git 未保存）。

**Phase C 詳細:** スキル `references/git-save.md`
...
11. [git-save.md](../git-save.md) — 触った各 Git ルートで `committing-with-git-yk` → `pushing-and-pr-yk`（push のみ）
12. セッション MD §2 と先頭表 `commit` 行を Post-C で更新
13. ユーザーへ commit hash · push 結果を報告（パス · 再開 `@` と合わせて）
...
| Phase B のみで commit/push しない | **否**（Git 未保存） |
| **確認** / **整理**での `git commit` / `git push` | 禁止（終了モード Phase C のみ） |
...
**禁止:** 最新セッション MD の削除 · HANDOFF 本文の丸ごと複製 · **整理/確認**モードでの `git commit` / `git push`
