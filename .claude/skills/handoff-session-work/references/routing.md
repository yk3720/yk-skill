...
**例外（検証駆動フェーズ）:** HANDOFF §6 に「検証駆動」「§4 機械消化しない」等がある slug（例: `flowchart-studio`）は **HANDOFF §6 が実行正本**。セッション §4 は履歴 — 検証メモまたは明示依頼があるまで §4 を実行しない。
...
11. `organizing-documents-yk` M1 Tier P — [reconcile.md](../../organizing-documents-yk/references/reconcile.md) S1–S7
12. 機械的矛盾は同一ターンで修正 · §1-3 に `資料整合（Phase B+）:` 1 行
