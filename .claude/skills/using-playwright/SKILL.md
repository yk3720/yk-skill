...
`c:\yk-memo\playwright-test` でのブラウザ自動化・E2Eテスト作成を支援する。
...
```
c:\yk-skill\rule\PLAYWRIGHT_RULES.md
```
...
- `c:\yk-skill\rule\PLAYWRIGHT_RULES.md` — 設計ルール・コマンドリファレンス・エラー対処（SSoT）
- `c:\yk-memo\playwright-test\` — テスト実行環境
- `c:\yk-memo\playwright-test\auth\session.json` — Google セッション（Git コミット禁止）

## ワークフロー

### Step 1: ルールを読む

`c:\yk-skill\rule\PLAYWRIGHT_RULES.md` を読み込む。

### Step 2: spec ファイルを作成・編集する

- `c:\yk-memo\playwright-test\tests\` 配下に `.spec.ts` として配置
...
cd c:\yk-memo\playwright-test; npx playwright test tests/<ファイル名>.spec.ts --reporter=line
```

> **初回 or 環境構築時**: ブラウザが未インストールの場合は先に実行する
> ```powershell
> cd c:\yk-memo\playwright-test; npx playwright install chromium
> ```
...
- `auth/session.json` を Git にコミットしない
